...
public class GUI extends JPanel implements MouseListener {
	...
	private void loadImages() {
		...
		// IDs:
		/*
		 * 0 = Pawn 1=Rook 2=Knight 3=Bishop 4=King 5=Queen
		 */
	...
	public void paintComponent(Graphics g) {
		...
		if(currentState.getBoard().inCheck) {
			drawInCheck(g2d);
		}
	}
	...
	private void drawBoard(Graphics2D g2) {
		...
		if (hastileSelected) {
			...
			if (currentState.getBoard().selectedTile.hasPiece() && currentState.getBoard().selectedTile.getPiece()
					.getColor() == currentState.getBoard().currentPlayer) {
				this.hastileSelected = true;
				ArrayList<Tile> Moves = currentState.getBoard().selectedTile.getPiece()
						.getValidMoves(currentState.getBoard());

				g2.setColor(Color.pink);
				for (Tile element : Moves) {
					g2.drawRect(element.column * TileSize, element.row * TileSize, TileSize, TileSize);
				}
	...
	private void drawPieces(Board b, Graphics2D g2) {
		...
		for (Piece element : P) {
			...
			if (element != null) {
				...
				// System.out.println("Draw: " + T[i][i].getColumn()*TileSize + " , " +
				// T[i][i].getRow() + " Value: " + T[i][i].getPiece().getColor());
				// g2.fillRect(T[i][i].getColumn()*TileSize, T[i][i].getRow()*TileSize,
				...
				g2.drawImage(imagePieces[a1][a2], element.getColumn() * TileSize, element.getRow() * TileSize, TileSize,
						TileSize, null);
				g2.setColor(Color.black);
			}
	...
	private void drawPlayerString(Graphics2D g2d) {
		...
		if (currentState.getBoard().currentPlayer) {
			currentPlayer = "White";
		} else {
			currentPlayer = "Black";
		}
	...
	private void drawInCheck(Graphics2D g2d) {
		g2d.drawString("This move results in check", 200, d.width + 100);
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getX() < d.width && e.getY() < d.height) {
			...
			if (!hastileSelected) { // tile is not selected, select it
				selectTile(e.getX(), e.getY());
				hastileSelected = true;

			} else {
				if (currentState.getBoard().getselectedTile().hasPiece()) {
					ArrayList<Tile> T = currentState.getBoard().getselectedTile().getPiece()
							.getValidMoves(currentState.getBoard());
					...
					for (Tile element : T) {
						if (element.getRow() == AttemptedTile.getRow()
								&& AttemptedTile.getColumn() == element.getColumn()) {
